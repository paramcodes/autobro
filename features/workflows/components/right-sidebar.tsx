...
function Palette() {
  ...
  const add = (type: NodeType) => {
    ...
    // Only one trigger allowed on the canvas.
    if (
      def.kind === "trigger" &&
    ...
    // Number same-type nodes (Open URL 1, Open URL 2, ...) to tell them apart.
    // Triggers are singletons so they keep their base label.
    const sameTypeCount = nodes.filter(
      (node) => node.data.type === type
    ).length
    const title =
      def.kind === "trigger" ? def.label : `${def.label} ${sameTypeCount + 1}`

    // Middle of the current view: canvas center in screen pixels -> flow coords.
